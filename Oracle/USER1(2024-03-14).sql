...
SELECT * FROM all_tables WHERE owner = 'USER1'; -- 대문자로 써야함

-- 현재 접속한 사용자 스키마에 등록된 table
SELECT * FROM user_tables;

-- DBA 권한으로 볼수 있는 table;
SELET * FROM dba_tables;
...
/*
 ...
 유니코드 문자열 : nCHAR, nVARCHAR
    영 숫자 이외의 2byte 형 문자(한글, 한자, 아랍어 등)를 저장하는 칼럼
...
/*
 ...
 전통적으로 Work Table 에는 PK 는 정수형으로 설정하고,
    일련번호를 부여하는 방식을 많이 사용한다.
    대부분의 DB 에는 AUTO INCREMENT 속성이 있어서 PK 칼럼을 설정한다.
    하지만, Oracle 11 이하의 버전에는 AUTO INCREMENT 속성이 아예 없다.
