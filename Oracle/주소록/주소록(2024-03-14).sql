...
-- 이름 칼럼에 민 이란 단어가 포함된 리스트만 SELECTION
...
-- 중복된 값 없이 table을 합치는 UNION
-- UNION 을 사용하려면 테이블이 같아야함
SELECT * FROM
(
...
SELECT COUNT(*) FROM tbl_addr_hoby;
...
-- EQ JOIN
SELECT A.*, AH.r_hcode ,H.h_name FROM tbl_address A
...
*/
-- 전체 데이터가 몇개니? 단독으로 사용될땐 GROUP BY 필요 x 
-- 조건 없이 전체 데이터의 개수
...
SELECT COUNT(*) FROM view_address
GROUP BY r_hcode, h_name; -- r_hcode랑 h_name 같은게 있으면 그룹으로 묶고 개수를 계산
...
SELECT h_name 취미명, r_hcode 취미코드, COUNT(*) 횟수 FROM view_address
GROUP BY h_name, r_hcode;
...
-- 다중 OR 조건문을 간소화 하는 키워드를 찾아 SQL 문을 작성
SELECT a_name, r_hcode, h_name FROM view_address
WHERE h_name IN ('마술', '사격', '쇼핑', '스쿼시')
ORDER BY h_name;

-- 취미가 '마술', '사격', '여행' 인 사람들의 참여 인원수를 계산하기
SELECT h_name, r_hcode,  COUNT(r_hcode)
FROM view_address
