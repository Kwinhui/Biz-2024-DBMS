...
*/
-- myDB라는 이름의 tablespace를 만들고 그 db는 'c:/app/data/myDB.dbf' 로 저장하겠다.
-- 초기크기 1mb 부족하면 1kb 씩 늘려라

-- myDB 라는 TableSpace 를 만들어라
-- 물리적 저장소는 c:/app/data/myDB.dbf 라는 파일로 만들어라
-- 초기 크기는 1M Byte 저장소를 설정하고
-- 만약 저장소 크기가 부족하면 1K Byte 씩 자동으로 확장하라 1MB > B를 적으면 x
...
-- oracle 고유명령어
-- mydb를 삭제하는데 포함된 컨텐츠, 데이터파일, 제약조건 모두 삭제, 드롭할때는 옵션들 모두 적어줘야함
...
*/
-- 오라클 12c 이전의 표준 SQL 명령으로 사용자 생성을 할수 있도록 하는 명령
...
/*
 ...
 사용자에게 부여한 권한은 표준화(조직내에서)된 정책을 수립하고,
 그 정책에 따라 세부적으로 권한을 부여하는것이 좋다.
...
CREATE TABLE user1.tbl_user (
    seq NUMBER,
    username VARCHAR2(125), -- VARCHAR2영문이나 숫자
    nickname nVARCHAR2(125) -- nVARCHAR2한글을 포함한 알파벳 이외의 글자
);
...
-- user1 에게 새로운 테이블을 생성, 삭제, 변경 하는 권한을 부여하기
