...
백업하기
	...
	백업할 Database 와 저장할 폴더를 선택한다
	전체 다 백업할 경우 sys 제외 모두 선택
...
/*
JOIN 통한 View 의 생성
물리적인 2개이상의 테이블을 서로 연계하여 보고자 하는 모양의 View Table 을
출력하는 SQL
*/

-- EQ JOIN(완전 JOIN)을 이용한 테이블 연결
-- EQ JOIN을 사용하려면 반드시 참조무결성, 외래키 설정이
-- 완전하게 이루어져야 한다.
SELECT io_seq, io_date, io_time, io_pcode, p_name, p_item, p_comp, -- products 정보
	io_dcode, d_name, d_ceo, d_tel, d_addr, -- depts 정보
...
참조무결성이 되지 않은 Table 을 EQ JOOIN 으로 연계하는 경우
보여지는 데이터가 누락되거나, 전혀 엉뚱한 리스트가
보여질 수 있다.

이때는 EQ JOIN 이 아닌 OUTTER JOIN 이나 INNER JOIN 을
사용해야 한다.
...
FROM tbl_iolist
	LEFT JOIN tbl_depts
		ON io_dcode = d_code;
/*
관리자(root) - DBA가 사용하는 명령어
DDL(물리적 CREATE(생성), ALTER(변경), DROP(삭제))
...
CREATE TABLE [식별자]
*/ 
-- 방금 실행했던 SELECT 명령문을 이용해 iolist 라는 view를 만들겠다.
-- VIEW : 실제테이블이 아닌 보는 용도에 가상테이블
...
VIEW 를 생성할 때, "Projection 을 적절히 수행"하여
...
SELECT io_pcode AS 상품코드, p_name AS 상품명,
...
GROUP BY io_pcode
HAVING 매입합산 > 536199
-- 통계함수(SUM) 에 의해서 만들어진 HAVING 함수
-- 매입합산이 536199 보다 큰 결과만 보여줘
-- HAVING 함수를 사용하면 SELECT문이 느려짐
ORDER BY 매입합산 DESC, 매출합산 DESC;

-- 매입매출 데이터에서 매입항목(io_div = 1)만 SELECT 하여
-- 상품코드, 상품명, 매입합계를 출력
-- 전체 500개 데이터 > 그룹 > sum > div = '1' 인 친구만 출력
SELECT io_div,
SUM(io_quan * io_iprice) AS 매입합산,
SUM(io_quan * io_oprice) AS 매출합산
FROM view_iolist
...
-- 전체 데이터 가져와서 div = 1인 친구만 추출 후 그룹 > 합산
SELECT io_div,
SUM(io_quan * io_iprice) AS 매입합산,
SUM(io_quan * io_oprice) AS 매출합산
FROM view_iolist
