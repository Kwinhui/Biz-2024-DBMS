...
미리 테이블에 제약조건을 설정하고, 데이터에 추가, 수정, 삭제를 할때 적용한다.

외래키는 1:N 관계에서 N 이 테이블에 설정한다.
1은 조건 테이블의 PK 와 연결한다.
*/
-- 매입매출과 상품 테이블의 FK 설정
-- 매입매출과 상품은 N:1의 관계이다
-- N 인 매입매출 Table 에 FK 설정한다
-- tbl_iolist 변경
-- tbl_iolist 와 tbl_products 를 서로 연결
