...
수정이상
	데이터를 변경(수정)할때 일부 데이터가 수정되지 않거나,
    수정되어서는 안되는 데이터가 변경되는 현상
삭제이상
	데이터를 삭제할때 일부 데이터가 삭제되지 않거나,
    삭제되어서는 안되는 데이터가 삭제되는 현상
    
수정, 삭제 이상은 PK 가 아닌 다른 칼럼을 조건으로
	하여 실행하는 과정에서 발생한다.
목적 : 이러한 이상(삽입, 수정, 상제)현상을 방지하기 위하여 정규화를 진행한다.
*/
...
SELECT io_pname,
ROUND(AVG(io_iprice),0) AS 매입단가, -- ROUND(AVG(io_price),0)소수점이하 첫번째자리에서 반올림
ROUND(AVG(io_oprice),0) AS 매출단가
FROM tbl_iolist
GROUP BY io_pname -- 중복없이
ORDER BY io_pname; -- 이름순
...
-- iolist에 pcode 와 products 에 있는 pcode가 일치하면 보여줘
...
-- iolist 전체 나열
-- io_pcode = p_code 일치한게 있으면 데이터를 연결해서 보여줘
SELECT COUNT(*)
FROM tbl_iolist, tbl_products
WHERE io_pcode = p_code;
...
-- 왼쪽에 있는 테이블은 무조건 다 출력 io_list = 500개
-- 오른쪽에 있는 테이블에서 SELECT 해서
-- io_pcode = p_code 비교해서 SELECT 해라
-- 상품이 있으면 상품이름 표시, 없으면 null 표시
/*
LEFT (OUTER) JOIN
이 키워드를 기준으로 왼쪽에 있는 Table(tbl_iolist) 은
무조건 다 나열을 해라
오른쪽의 테이블에서 ON 의 조건에 맞는 데이터를 SELECT 해라
...
-- LEFT JOIN 을 실행한 결과에서 오른쪽 테이블에서 
-- SELECT 한 값이 NULL 인 경우가 발생하면 JOIN 한
-- 테이블은 "참조 무결성"이 깨진것이다.
SELECT io_pcode, p_name
FROM tbl_iolist
LEFT JOIN tbl_products
